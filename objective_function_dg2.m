function [Fitness,LOSS,V_DEV,Bus00]=objective_function_dg2(x,Buses,Lines,senarios,M)
    Bus0=Buses;
    Line0=Lines;
    x(:,6)=round(x(:,6)*size(Buses,1)/size(Lines,1));
    x(:,8)=round(x(:,8)*size(Buses,1)/size(Lines,1));
    x(:,7)=round(x(:,7)*4/37);
    x(:,9)=round(x(:,9)*4/37);
    ...
    for m=1:M 
        ...
        if redial_index~=0
            for s=1:size(senarios,1)
                ...
                Buses(x(m,6),2)=Buses(x(m,6),2)-x(m,7);
                Buses(x(m,6),3)=Buses(x(m,6),3)-(3/4)*x(m,7);
                Buses(x(m,8),2)=Buses(x(m,8),2)-x(m,9);
                Buses(x(m,8),3)=Buses(x(m,8),3)-(3/4)*x(m,9);
                ...
                for jj=1:33
                    Voltage_dev(jj)=abs(1-abs(Bus(jj)));
                end
                Loss1(s)=sum(Loss);
